...
// ─── CATEGORY / SUBCATEGORY (Rewix tags are Italian, no en_US locale) ────────
const CATEGORY_EN = {
  scarpe: 'Shoes',
...
const SUBCATEGORY_EN = {
  sneakers: 'Sneakers', maglie: 'Knitwear', costumi: 'Swimwear',
  'borse a tracolla': 'Crossbody Bags', camicie: 'Shirts', 't-shirt & top': 'T-Shirts & Tops',
  abiti: 'Dresses', abiiti: 'Dresses',               // 'abiiti' = feed typo for 'abiti'
  giacche: 'Jackets', gonne: 'Skirts', completi: 'Suits', cappotti: 'Coats',
  zaini: 'Backpacks', marsupi: 'Belt Bags', pochette: 'Clutches', 'borse a spalla': 'Shoulder Bags',
  borse: 'Bags', 'giacca classica': 'Blazers', pantaloni: 'Trousers', shorts: 'Shorts',
  'borse a mano': 'Handbags', felpe: 'Sweatshirts', gilet: 'Vests', polo: 'Polo Shirts',
  cappelli: 'Hats', jeans: 'Jeans', bluse: 'Blouses', leggings: 'Leggings', tute: 'Tracksuits',
  body: 'Bodysuits', intimo: 'Underwear', sciarpe: 'Scarves', scaldamuscoli: 'Leg Warmers',
  stivaletti: 'Ankle Boots', stivali: 'Boots', 'shopping bag': 'Shopping Bags', valigie: 'Luggage',
  zainetti: 'Small Backpacks', portamonete: 'Coin Purses', portacarte: 'Card Holders',
};

// Translate a category tag. Falls back to the word translator, then to a
// capitalized original so nothing is ever lost.
function translateCategory(raw) {
  if (!raw) return '';
  const k = raw.trim().toLowerCase();
  if (CATEGORY_EN[k]) return CATEGORY_EN[k];
  const t = translatePhrase(raw);
  return t || (raw.charAt(0).toUpperCase() + raw.slice(1));
}

function translateSubcategory(raw) {
  if (!raw) return '';
  const k = raw.trim().toLowerCase();
  if (SUBCATEGORY_EN[k]) return SUBCATEGORY_EN[k];
  const t = translatePhrase(raw);
  return t || (raw.charAt(0).toUpperCase() + raw.slice(1));
}

// ─── MADE-IN NORMALIZATION ────────────────────────────────────────────────────
// Romanelli's feed has 33 spelling variants for ~20 real countries.
// All comparisons are case- and whitespace-insensitive.
const MADE_IN_MAP = {
  // Italy
  'italy': 'Italy', 'italia': 'Italy',
  // China
  'china': 'China', 'cina': 'China', 'prc': 'China',
  // Turkey
  ...
  // Bulgaria
  'bulgaria': 'Bulgaria', 'bulgary': 'Bulgaria',
  // Ukraine
  'ukraine': 'Ukraine', 'ucraina': 'Ukraine',
  // Cambodia
  'cambodia': 'Cambodia', 'cambogia': 'Cambodia',
  // Straightforward (normalise casing/spacing only)
  'albania': 'Albania', 'bangladesh': 'Bangladesh', 'czech republic': 'Czech Republic',
  'india': 'India', 'indonesia': 'Indonesia', 'mauritius': 'Mauritius',
  'mongolia': 'Mongolia', 'myanmar': 'Myanmar', 'poland': 'Poland',
  'portugal': 'Portugal', 'romania': 'Romania', 'slovakia': 'Slovakia',
  'tunisia': 'Tunisia', 'vietnam': 'Vietnam',
};

function normalizeMadeIn(raw) {
  if (!raw) return '';
  const key = raw.trim().toLowerCase();
  return MADE_IN_MAP[key] || raw.trim(); // unknown → pass through trimmed
}

// ─── SEASON NORMALIZATION ─────────────────────────────────────────────────────
// Romanelli mixes English, Italian, and blank values for Stagione.
const SEASON_MAP = {
  'spring/summer': 'Spring/Summer',
  'fall/winter':   'Fall/Winter',
  'all year':      'All Year',
  // Italian variants found in the feed
  'primavera-estate': 'Spring/Summer',
  'primavera/estate': 'Spring/Summer',
  'continuativo':     'All Year',
...
function normalizeSeason(raw) {
  ...
  const key = raw.trim().toLowerCase();
  return SEASON_MAP[key] || raw.trim();
}

// ─── GENDER NORMALIZATION ─────────────────────────────────────────────────────
// Feed has confirmed typo 'uoomo' and may have other variants.
const GENDER_MAP = {
  'woman': 'Woman', 'women': 'Woman',
  'man': 'Man', 'men': 'Man', 'uoomo': 'Man', 'uomo': 'Man',
  'junior': 'Junior', 'kids': 'Junior', 'bambino': 'Junior', 'bambina': 'Junior',
  'unisex': 'Unisex',
};
...
module.exports = {
  translatePhrase, buildTitle, GARMENTS, COLORS,
  translateCategory, translateSubcategory, CATEGORY_EN, SUBCATEGORY_EN,
  normalizeMadeIn, normalizeSeason, normalizeGender,
};
