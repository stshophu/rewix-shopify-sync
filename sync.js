...
async function loadShopifyIndex() {
  ...
  let url = `${shopifyBase()}/products.json?limit=250&fields=id,variants,tags,status,metafields`;
  while (url) {
    ...
    for (const p of data.products || []) {
      ...
      // Authoritative bare-code key: the rewix.product_code metafield, written
      // on every create. The previous approach — stripping the last "-segment"
      // off the SKU to reconstruct m.code — silently breaks whenever the Rewix
      // size token itself contains a hyphen (e.g. size "7-XXL" → SKU
      // "...-bianco-7-XXL" → stripped to "...-bianco-7", NOT the real bare code
      // "...-bianco"). That mismatch is what caused match strategy 1 to keep
      // missing existing products and recreate them as duplicates on every run.
      const productCodeMeta = (p.metafields || [])
        .find(mf => mf.namespace === 'rewix' && mf.key === 'product_code')?.value;
      ...
      for (const v of p.variants || []) {
        ...
        if (productCodeMeta && !skuToProductId.has(productCodeMeta)) {
          skuToProductId.set(productCodeMeta, p.id);
        } else if (!productCodeMeta) {
          // Legacy fallback only for products created before this metafield
          // existed (so the size-suffix migration era still resolves).
...
async function run() {
  ...
  for (let i = 0; i < products.length; i++) {
    ...
    let existingId = firstModel?.code ? skuToProductId.get((firstModel.code || '').trim()) : null;
